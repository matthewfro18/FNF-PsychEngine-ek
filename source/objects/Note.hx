...
class Note extends FlxPerspectiveSprite
{
	...
	public static var noteScales:Array<Float> = [0.7, 0.6, 0.5, 0.65, 0.58, 0.55, 0.7, 0.7, 0.7, 0.40, 0.35];
	public static var pixelNoteScales:Array<Float> = [1, 0.83, 0.7, 0.9, 0.8, 0.74, 1, 1, 1, 0.65];
	public static var noteWidths:Array<Float> = [112, 84, 66.5, 91, 77, 70, 140, 126, 119, 55, 48];
	public static var sustainXOffsets:Array<Float> = [97, 84, 70, 91, 77, 78, 97, 97, 97, 65, 50];
	public static var posRest:Array<Int> = [0, 35, 70, 0, 50, 60, 0, 0, 0, 70, 80];
	...
	public static var frameN:Array<Dynamic> = [
		...
		['purple', 'red'],
		['purple', 'white', 'red'],
		['purple', 'blue', 'green', 'red', 'white','orange','yellow', 'violet', 'darkred', 'dark'],
		['pink', 'emerald', 'lightred','purple', 'green', 'red', 'yellow', 'blue', 'dark', 'lime', 'turq', 'cobalt']
	];
	public static var keyAmmo:Array<Int> = [4, 6, 9, 5, 7, 8, 1, 2, 3, 10, 12];
	...
	public function new(strumTime:Float, noteData:Int, ?prevNote:Note, ?sustainNote:Bool = false, ?inEditor:Bool = false)
	{
		...
		swagWidth = Note.noteWidths[mania];
		if (prevNote == null)
			prevNote = this;
	...
	function loadNoteAnims(prefix:String = '') {
		...
		switch(prefix)
		{
			case 'DEATH': //too lazy to edit xmls
				...
				{
					animation.addByPrefix(frameN[mania][i] + 'Scroll', 'death0'); // Normal notes
					animation.addByPrefix(frameN[mania][i] + 'hold', 'death hold piece'); // Hold
					animation.addByPrefix(frameN[mania][i] + 'holdend', 'death hold end'); // Tails
				}
			case 'WARNING':
				for (i in 0...Note.keyAmmo[mania])
				{
					animation.addByPrefix(frameN[mania][i] + 'Scroll', 'warning0'); // Normal notes
					animation.addByPrefix(frameN[mania][i] + 'hold', 'warning hold piece'); // Hold
					animation.addByPrefix(frameN[mania][i] + 'holdend', 'warning hold end'); // Tails
				}
		...
		if (isSustainNote)
			setGraphicSize(Std.int(width * noteScales[mania]), Std.int(width * 0.7));
		else 
			setGraphicSize(Std.int(width * noteScales[mania]));
		updateHitbox();
	}
